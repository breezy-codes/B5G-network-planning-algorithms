...
This is the main script for running the Graph-Encoded Genetic Algorithm (GEGA) on the given dataset.
It sets up the environment, loads the necessary data, initialises the units, and runs GEGA.
...
You can also adjust the parameters for GEGA to change its behaviour.
...
from modules.algorithms.genetic_algorithm import save_GEGA
...
results_dir = os.path.join(current_dir, "results", "GEGA", dataset)
